...
public class PerksSurvivorActivity extends AppCompatActivity {
    ...
    private ButtonAdapterPerks buttonAdapterPerks; // Declare the adapter for buttons
    private List<PerkItem> buttonItems; // Declare the list of button items
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_perks_killer);
        ...
        // Set the query hint programmatically to "Search perks..."
        searchView.setQueryHint("Search perks...");
        ...
        int itemWidth = getResources().getDimensionPixelSize(R.dimen.grid_item_width_perks); // Button width
        ...
        recyclerView.addItemDecoration(new GridSpacingItemPerks(spanCount, spacing, true));

        // Get the list of ButtonItems from the ButtonItemData class
        buttonItems = PerkSurvivorItemData.getButtonItems();

        // Initialize the adapter and set it to the RecyclerView
        buttonAdapterPerks = new ButtonAdapterPerks(this, buttonItems);
        recyclerView.setAdapter(buttonAdapterPerks);
    ...
    private void applyFilters() {
        List<PerkItem> filteredItems = new ArrayList<>();

        // Filter the items based on the current search query (search both labels and tags)
        for (PerkItem item : buttonItems) {
            boolean matchesSearchQuery = item.getLabel().toLowerCase().contains(currentSearchQuery.toLowerCase());

            // Check if any of the tags also match the search query
            for (String tag : item.getTags()) {
                if (tag.toLowerCase().contains(currentSearchQuery.toLowerCase())) {
        ...
        buttonAdapterPerks.updateItems(filteredItems);
    }
}
...
public class CharactersSurvivorActivity extends AppCompatActivity {

    private ButtonAdapterCharactersSurvivor buttonAdapterCharactersSurvivor; // Declare the adapter for buttons
    private List<CharacterItem> buttonItems; // Declare the list of button items
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_characters_survivor);

        // Reference to SearchView
        SearchView searchView = findViewById(R.id.search_view_character);
        ...
        // Set the query hint programmatically to "Search survivors..."
        searchView.setQueryHint("Search survivors...");
        ...
        int itemWidth = getResources().getDimensionPixelSize(R.dimen.grid_item_width_characters); // Button width
        ...
        recyclerView.addItemDecoration(new GridSpacingItemCharacters(spanCount, spacing, true));

        // Get the list of ButtonItems from the ButtonItemData class
        buttonItems = CharacterSurvivorItemData.getButtonItems(this);

        // Initialize the adapter and set it to the RecyclerView
        buttonAdapterCharactersSurvivor = new ButtonAdapterCharactersSurvivor(this, buttonItems);
        recyclerView.setAdapter(buttonAdapterCharactersSurvivor);
        ...
        spinner.setSelection(2); // Index 2 corresponds to the third item ("Old-New")
    ...
    private void applyFilters() {
        List<CharacterItem> filteredItems = new ArrayList<>();

        // Filter the items based on the current search query
        for (CharacterItem item : buttonItems) {
            boolean matchesSearchQuery = item.getLabel().toLowerCase().contains(currentSearchQuery.toLowerCase());

            // If either the label or the tags match the search query, include the item
        ...
        buttonAdapterCharactersSurvivor.updateItems(filteredItems);
    }
}